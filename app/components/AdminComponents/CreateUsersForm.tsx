...
export default function CreateUsersForm({ sponsors, action }: CreateUsersFormProps) {
  const [userType, setUserType] = useState<"driver" | "sponsor">("driver");
    ...
    <form action={action} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <select
          name="userType"
          value={userType}
          onChange={(e) => setUserType(e.target.value as "driver" | "sponsor")}
          required
      ...
      <div className="md:col-span-2">
        ...
        >
          Create {userType === "driver" ? "Driver" : "Sponsor User"}
        </button>
      </div>
    </form>
  );
