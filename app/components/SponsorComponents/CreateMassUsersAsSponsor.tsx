...
function parseFile(content: string, userRole: "admin" | "sponsor" = "sponsor"): ParseResult {
  ...
  lines.forEach((raw, idx) => {
    ...
    // Sponsor-specific rule: cannot use O type
    if (userRole === "sponsor" && type === "O") {
      errors.push({ line: lineNum, message: `Sponsors cannot create organizations`, raw });
      return;
    }

    // O type only needs 2 fields: O|Org Name
    if (type === "O") {
      if (parts.length < 2 || !parts[1]) {
        errors.push({ line: lineNum, message: `Organization record missing name`, raw });
        return;
      }
      valid.push({ line: lineNum, type, orgName: parts[1], firstName: "", lastName: "", email: "" });
      return;
    }

    // D and S: type|orgName|firstName|lastName|email|points?|reason?
    let orgName: string, firstName: string, lastName: string, email: string, pointsRaw: string, reason: string;
    if (userRole === "sponsor") {
    // Sponsor format: type|firstName|lastName|email|points?|reason?
    [, firstName, lastName, email, pointsRaw, reason] = parts;
    orgName = "";
    } else {
    // Admin format: type|orgName|firstName|lastName|email|points?|reason?
    [, orgName, firstName, lastName, email, pointsRaw, reason] = parts;
    }
    if (!firstName) {
      errors.push({ line: lineNum, message: `Missing first name`, raw });
      return;
    }
    if (!lastName) {
      errors.push({ line: lineNum, message: `Missing last name`, raw });
      return;
    }
    if (!email || !email.includes("@")) {
      errors.push({ line: lineNum, message: `Invalid or missing email "${email ?? ""}"`, raw });
      return;
    }

    // Sponsor rule: error if orgname present and there is a sponsor. It should automatically have the sponsors organixation
    if (userRole === "sponsor" && orgName) {
        errors.push({
            line: lineNum,
            message: `Organization name "${orgName}" is not allowed for sponsor uploads`,
            raw,
        });
        return;
    }

    // Sponsor rule: points cannot be assigned to sponsor users
    if (userRole === "sponsor" && type === "S" && pointsRaw) {
    errors.push({
        line: lineNum,
        message: `Points are not allowed for sponsor users ("${email}")`,
        raw,
    });
    return;
    }
    ...
    if (pointsRaw && !reason) {
      errors.push({ line: lineNum, message: `Points provided but reason is missing for "${email}"`, raw });
      return;
    }
    if (pointsRaw && isNaN(Number(pointsRaw))) {
      errors.push({ line: lineNum, message: `Points "${pointsRaw}" is not a valid number for "${email}"`, raw });
      return;
    }
...
export default function CreateMassUsersAsSponsor({ userRole = "sponsor" }: { userRole?: "admin" | "sponsor" }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = parseFile(content, userRole);
      setParsed(result);
      setStatus("preview");
    } catch {
