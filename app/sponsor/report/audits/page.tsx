...
// Categories that route to a dedicated report page
const ROUTED_CATEGORIES: Partial<Record<AuditCategory, string>> = {
  "Login Attempts":  "/sponsor/report/audits/login-attempts-report",
  "Point Change":    "/sponsor/report/audits/point-change-report",
  "Password Change": "/sponsor/report/audits/password-change-report",
  "Driver Status":   "/sponsor/report/audits/driver-status-report",
};
...
export default function SponsorAuditReportPage() {
  const router = useRouter();
 
  // Resolved from the current user session — not a user-facing filter
  const [currentSponsorName, setCurrentSponsorName] = useState<string | null>(null);
  ...
  // Resolve the current user's sponsor name on mount
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/user/sponsor/organization");
        if (res.ok) {
          const data = await res.json();
          setCurrentSponsorName(data.sponsorName ?? null);
        }
      } catch {
        setCurrentSponsorName(null);
      } finally {
  ...
  const handleGenerate = async () => {
    ...
    const sponsor = currentSponsorName ?? "All Sponsors";
    ...
    setResults(entries);
    setAppliedFilters({ dateFrom, dateTo, category: selectedCategory });
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5 pt-24">
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-slate-900">Sponsor Reports</h1>
            {/* Sponsor identity pill */}
            {!sponsorLoading && (
              <div className="flex items-center gap-1.5 mt-1">
                <span className="text-xs text-slate-400">Scoped to</span>
                {currentSponsorName ? (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-indigo-50 border border-indigo-200 text-xs font-medium text-indigo-700">
                  ...
                  <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-xs text-slate-500">
                    Unknown sponsor
                  </span>
                )}
              </div>
      ...
      <div className="max-w-screen-xl mx-auto px-8 py-6 space-y-5">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            {/* Sponsor scope — read-only */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700">Sponsor scope</label>
              <div className="h-9 w-full px-3 flex items-center gap-2 border border-slate-200 rounded-lg bg-slate-50 text-sm text-slate-500 select-none">
                {sponsorLoading ? (
                  ...
                  <>
                    ...
                    <span className="text-slate-700 font-medium">{currentSponsorName}</span>
                    <span className="ml-auto text-xs text-slate-400 italic">auto-applied</span>
                  </>
                ) : (
                  <span className="text-slate-400 text-xs italic">Sponsor not resolved</span>
                )}
              </div>
              <p className="text-xs text-slate-400">Automatically scoped to your organization.</p>
            </div>
          </div>
            ...
            >
              {CATEGORIES.map((cat) => {
                ...
                return (
                  ...
                  >
                    {cat}
                    {isRouted && <span className="ml-1.5 text-[10px] opacity-50">↗</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex items-center justify-between pt-1 border-t border-slate-100">
            ...
            <button
              ...
              disabled={!canGenerate || loading || sponsorLoading}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                canGenerate && !loading && !sponsorLoading
        ...
        {!appliedFilters && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-xs">
              Choose a date range and a category above, then click Generate report.
            </p>
          </div>
        )}
        ...
        {appliedFilters && results && metrics && (
          <>
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-slate-100 border border-slate-200 rounded-lg text-xs text-slate-600">
              ...
              {currentSponsorName && (
                <span className="px-2 py-0.5 bg-indigo-50 border border-indigo-200 text-indigo-700 rounded-md font-medium">
                  {currentSponsorName}
                </span>
              )}
