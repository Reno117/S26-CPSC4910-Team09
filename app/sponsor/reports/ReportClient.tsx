...
export default function SponsorAudientClient({ sponsorId }: { sponsorId: string }) {
  ...
  const handleGenerate = async () => {
    ...
    // Route to dedicated page if one exists, passing the sponsor ID from session
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="max-w-screen-xl mx-auto px-8 py-6 space-y-5">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-6">
          ...
          {/* Date range (full width since sponsor selector is removed) */}
          ...
          {/* Driver filter — only shown when Sales by Driver is selected */}
          {selectedCategory === "Sales by Driver" && (
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-slate-400">Required for Sales by Driver. Supports suggestions while typing.</p>
            </div>
          )}
        ...
        {appliedFilters && results && metrics && (
          <>
            ...
            {/* Metric cards — removed "Sponsors" card since this is scoped to one */}
