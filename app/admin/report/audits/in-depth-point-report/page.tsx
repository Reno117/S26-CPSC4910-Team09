...
import { FilterBar } from "@/app/components/AdminComponents/Point-Change-Filter";
 
// ─── Types ────────────────────────────────────────────────────────────────────
 
interface PageProps {
  searchParams: Promise<{ from?: string; to?: string; sponsor?: string; driver?: string; reason?: string }>;
}
...
export default async function PointChangeReportPage({ searchParams }: PageProps) {
  const { from, to, sponsor = "All Sponsors", driver = "", reason = "" } = await searchParams;
  ...
  const result = await getPointChangeAuditReport({ dateFrom: from, dateTo: to, sponsor, driver, reason });
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5">
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          ...
          <FilterBar
            drivers={result.driverSummaries}
          ...
          <ExportCSVButton
            ...
            fetchCSV={async () => {
              "use server";
              return exportPointChangeAuditCSV({ dateFrom: from, dateTo: to, sponsor, driver, reason });
            }}
          />
            </div>
        </div>
